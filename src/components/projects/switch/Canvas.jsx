...
function Switch({ x, set }) {
  ...
  const { projectPlatform } = appContext.state;

  const { nodes, materials } = useGLTF('./switch.glb');
  const texture = useTexture('./arrows.jpg');
  // Hover state
  ...
  const toggled = projectPlatform === 'web' ? 'mobile' : 'web';
  // Events
  const onClick = useCallback(() => {
    appContext.setProjectPlatform(toggled);
    set((toggle) => Number(!toggle)), [set];
  });
...
export function Scene({ x, set }) {
  ...
  return (
    <Canvas
      ...
      shadows
      dpr={[1, 2]}
      camera={{ zoom: 20, position: [10, 10, 10], fov: 135 }}
    >
